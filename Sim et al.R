...
Figure_6B
# Figure 6D Metagenomic Analysis ------------------------------------------
#Read and filter mapped reads 
fbeb_frd <- read.csv("MS-Metagenomic_data.csv")%>%
  select(!X)
#Load gene length information
gene_lengths <- read.csv("Query_gene_information.csv")%>%
  select(!X)
#Total reads found in each sample
total_reads <- read.csv("Total_Sample_Reads.csv")%>%
  select(!X)

mapq_min <- 30
filtered_reads <- fbeb_frd |>
  mutate(
...
#Count reads per sample and gene 
counts_by_sample <- filtered_reads |>
  dplyr::count(sample, rname, name = "reads") |>
  dplyr::arrange(sample, desc(reads))

counts_wide <- counts_by_sample |>
  pivot_wider(
    names_from = sample,
    values_from = reads,
    values_fill = 0) |>
...
counts_rpkm <- counts_wide |>
  ...
  mutate(rpkm = counts / ((length_bp / 1000) * (total_reads / 1e6))) |>
  select(locus_tag, sample, rpkm) |>
  pivot_wider(names_from = sample, values_from = rpkm)

counts_rpkm_matrix <- counts_rpkm |>
...
sample_order <- colnames(counts_rpkm_matrix) |>
  str_remove("ERR") |>
  as.numeric() |>
  order()

counts_rpkm_matrix <- counts_rpkm_matrix[gene_order, sample_order]


#Create heatmap
mat_log <- log2(counts_rpkm_matrix + 1)

breaks_fixed <- seq(
  min(mat_log, na.rm = TRUE),
  max(mat_log, na.rm = TRUE),
  length.out = 101)

gene_groups <- data.frame(gene = rownames(counts_rpkm_matrix))
rownames(gene_groups) <- gene_groups$gene

annotation_colors <- list(gene = gene_colors)
...
heatmap <- pheatmap(mat_log,
                    ...
                    color = colorRampPalette(
                      c("#26456E", "#4A6FE3", "white", "#D33F6A", "#9C0824")
                    )(100),
                    breaks = breaks_fixed,
                    main = "",
                    silent = TRUE)$gtable

Figure_6d <- ggplotify::as.ggplot(heatmap)
Figure_6d


#Plot RPKM distributions by gene
mean_rpkm_long <- counts_rpkm_matrix |>
  as.data.frame() |>
  rownames_to_column("locus_tag") |>
  pivot_longer(
    cols = -locus_tag,
...
Figure_6f <- ggplot(
  mean_rpkm_long,
  aes(x = locus_tag, y = rpkm, fill = locus_tag)) +
  geom_violin() +
  geom_jitter(
    width = 0.2,
    alpha = 0.4,
...
# Figure 6E Correlation Analysis --------------------------------------------------

human_cluster <- read.csv(file="RPKM_FBEB.csv",header=T,sep=",")



human_cluster %>%
  filter(sample!="mean_rpkm_FBEB")%>%
  select(locus_tag)%>%
  unique()

human_cluster_df <- human_cluster %>%
  select(-X)%>%
  spread(key="locus_tag",value = "rpkm")%>%
  select(-sample)

results_human <- rcorr(as.matrix(human_cluster_df),type="spearman")

results_human_r <- results_human$r
results_human_p <- results_human$P


corrplot(results_human_r, 
         p.mat = results_human_p,
